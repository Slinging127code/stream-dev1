...
import com.jiao.bean.TableProcessDwd;
...
/**
 * @Package com.cj.realtime.util.FlinkSinkUtil
 * @Author xinyi.jiao
 * @Date 2025/4/10 14:10
 * @description: 3
 */
public class FlinkSinkUtil {
    ...
    public static KafkaSink<Tuple2<JSONObject, TableProcessDwd>> getFlinkSinkUtil(){
    ...
    public static <T>KafkaSink<T> getFlinkSinkUtil(KafkaRecordSerializationSchema<T> ksr){
        KafkaSink<T> kafkaSink = KafkaSink.<T>builder()
                .setBootstrapServers(Constant.KAFKA_BROKERS)
                .setRecordSerializer(ksr)
                //当前配置决定是否开启事2务，保证写到kafka数据的精准一次
                //.setDeliveryGuarantee(DeliveryGuarantee.EXACTLY_ONCE)
                //设置事务Id的前缀
                //.setTransactionalIdPrefix("dwd_base_log_")
                //设置事务的超时时间   检查点超时时间 <     事务的超时时间 <=事务最大超时时间
